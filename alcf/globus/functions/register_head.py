...
# Globus Compute function definition (subprocess - head)
def head(params):
    ...
    # Make sure lines and file_bytes are not provided at the same time
    if input_data.file_bytes and input_data.lines:
        return Response(error=f"Cannot use 'file_bytes' and 'lines' at the same time.").model_dump()
    ...
    if input_data.file_bytes:
        if input_data.skip_trailing:
            cmd.extend(["-c", f"-{input_data.file_bytes}"])
        else:
            cmd.extend(["-c", str(input_data.file_bytes)])
    if input_data.lines:
        if input_data.skip_trailing:
            cmd.extend(["-n", f"-{input_data.lines}"])
        else:
            cmd.extend(["-n", str(input_data.lines)])
