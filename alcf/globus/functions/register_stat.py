...
# Globus Compute function definition (stat)
def stat(params):
    ...
    class FileStat(BaseModelWithForbiddenExtra):
    ...
    class Response(BaseModelWithForbiddenExtra):
        output: Optional[FileStat] = None
        error: Optional[str] = None

    # Pydantic for input data
    class InputData(BaseModelWithForbiddenExtra):
        path: Path
        dereference: Optional[bool] = Field(default=False)
    ...
    try:
        if statlib.S_ISLNK(st_path.st_mode):
    ...
    if o_nofollow is None:
        return Response(
            error="Platform does not support O_NOFOLLOW; TOCTOU-safe stat unavailable."
        ).model_dump()
    ...
    try:
        # Get metadata and verify target type is a regular file
        stat_before = os.fstat(fd)
        if not statlib.S_ISREG(stat_before.st_mode):
        ...
        output = FileStat(
            ...
            ino=stat_before.st_ino,
            dev=stat_before.st_dev,
            nlink=stat_before.st_nlink,
            uid=stat_before.st_uid,
            gid=stat_before.st_gid,
            size=stat_before.st_size,
            atime=int(stat_before.st_atime),
            ctime=int(stat_before.st_ctime),
            mtime=int(stat_before.st_mtime),
        )

        # Validate that the path was not changed during the operation
    ...
    # Error if something went wrong
        ...
        return Response(error=f"Could not execute stat command on path {path_str}.").model_dump()
...
COMPUTE_FUNCTION_ID = gcc.register_function(stat, public=True)
