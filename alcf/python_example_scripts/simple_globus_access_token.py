...
# ALCF Mapped Collection UUID
COLLECTION_ID = "05d2c76a-e867-4f67-aa57-76edeb0beda0"

# Creating our additional scope string to enable transfer operations on Mapped collection
additional_scopes = f"urn:globus:auth:scope:transfer.api.globus.org:all[*https://auth.globus.org/scopes/{COLLECTION_ID}/data_access]"
combined_scope=f"{SCOPE_STRING}[{additional_scopes}]"

# Prepare the authentication flow with the targetted scope
client = globus_sdk.NativeAppAuthClient(client_id=AUTH_CLIENT_ID)
client.oauth2_start_flow(requested_scopes=[combined_scope])
